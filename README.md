...
GrowthLab is an open-source SEO growth tool for comparing landing pages, content ideas, and onboarding experiments with inspectable local checks.
...
- Local cargo-dist shell and PowerShell installer templates generated and syntax-checked; macOS arm64 shell install smoke passed, while publication and other target runtime remain pending
...
- Installer template mapping smoke covers all five published shell targets and keeps Windows GNU away from unavailable MSVC assets
...
- Refined the GitHub About, README and Pages copy around the SEO growth tool positioning
...
- Hypothesis evidence validation requires complete provenance metadata, including confidence rationale and at least one linked claim, and rejects credential-shaped text before persistence

| Validation | Current evidence |
| --- | --- |
| Rust tests | **passed** — Targeted evidence-model suite: 7 tests per binary; 7 passed with zero failures. The preceding full serial suite on the parent source passed 970 tests per binary (968 passed, two inherited ignored); this change adds claim-link validation coverage. |
...
| Public release asset verification | **passed** — The read-only scripts/test-release-assets.py check downloads all ten alpha.20 archive/checksum pairs (five cargo-dist installer names plus five historical versioned names), matches GitHub asset digests and sidecars, and passes root-layout or bin-layout archive safety/notices verification; macOS arm64 runtime passes locally, macOS x86_64 passes under Rosetta, and Linux and Windows runtimes are skipped on the macOS host. |
| Local installer template generation | **passed** — cargo-dist generated the alpha.20 shell and PowerShell installer templates locally; sh -n and the PowerShell parser accepted both scripts. A separate local SHA-256 preflight matched the macOS arm64 archive and sidecar before the shell installer fetched that archive through a disposable HTTP server, installed growthlab and orx into an isolated prefix, and reported version 0.1.0-alpha.20 without editing the test profile. The scripts and cargo-dist-compatible archives are attached to the public release; other target installer/runtime execution remains unavailable. |
| Installer target matrix alignment | **passed** — The cargo-dist matrix now includes the published x86_64-pc-windows-gnu archive and leaves MSVC out until a Windows SDK runner can validate it. Regenerated PowerShell and shell templates select the GNU asset; mapping assertions, shell syntax, PowerShell parsing, public ten-pair SHA-256 verification and an isolated macOS shell-install smoke passed. |
| Installer template mapping smoke | **passed** — scripts/test-installer-templates.py --check-shell is versioned and checks that the generated shell template selects the published macOS arm64, macOS x86_64, Linux musl arm64, Linux musl x86_64 and Windows GNU archives; the PowerShell template selects the Windows GNU asset, with shell syntax and PowerShell parsing passing without network, upload or provider access. |
...
| Public SEO positioning | **passed** — GitHub About metadata, the README opening, and the live Pages title, description and hero were verified to use consistent SEO growth tool positioning. This is messaging and discovery evidence, not ranking or conversion proof. |
...
| Windows MSVC build probe | **unverified** — A local cargo-zigbuild attempt stopped in ring's C build because assert.h and a Windows SDK are unavailable on the macOS host; no MSVC artifact was produced or published. |
| Installer-compatible public release assets | **passed** — Five cargo-dist archive/checksum pairs were generated with the configured .tar.gz Unix format, root-level growthlab/orx binaries and six attribution/dependency notices; a deterministic Windows GNU ZIP was built with Zig. All ten public archive/checksum pairs passed the read-only verifier. Linux and Windows runtime, MSVC packaging and signing remain unverified. |
...
- Linux/Windows runtime proof, MSVC support and portable installers
<!-- project-status:end -->
