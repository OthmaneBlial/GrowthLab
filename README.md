...
| Local installer template generation | **passed** — cargo-dist generated the alpha.20 shell and PowerShell installer templates locally with no upload; sh -n accepted the 55,250-byte shell script and the PowerShell parser accepted the 22,419-byte script. The shell installer also fetched the local aarch64-apple-darwin archive through a disposable HTTP server, verified its SHA-256 sidecar, installed growthlab and orx into an isolated prefix, and reported version 0.1.0-alpha.20 without editing the test profile. Other target installer/runtime execution remains unavailable. |
