...
import { runPhase, selectedVariant, variantRun } from "./view";
import "./growth.css";
...
export function GrowthDashboard({ battleId, projectId }: { battleId?: string; projectId?: string }) {
  ...
  const [tab, setTab] = useState<"evidence" | "artifacts">("evidence");
  ...
  function tabKey(event: KeyboardEvent<HTMLButtonElement>) {
    ...
    const next = event.key === "Home" ? "evidence" : event.key === "End" ? "artifacts" : tab === "evidence" ? "artifacts" : "evidence";
    setTab(next);
    (next === "evidence" ? evidenceTab : artifactsTab).current?.focus();
  }
  ...
  async function importProduct(event: React.FormEvent) {
    event.preventDefault();
    const result = await perform("Importing product", () => growth.import(importPath.trim()));
