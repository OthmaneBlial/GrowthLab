...
import { growth, type ApplyPreview, type Execution, type Hypothesis, type HypothesisUpdate, type Provenance, type GrowthPlaybook, type PlaybookRun, type PublicRepositoryAudit, type RemoteSeoAudit, type SeoRubric, type PageQualityRubric, type RenderRubric, type PerformanceRubric, type AccessibilityRubric } from "./api";
...
type LocalizedPlaybook = { title: string; focus: string; summary: string; questions: string[]; outputs: string[]; guardrails: string[] };
function localizedPlaybook(item: GrowthPlaybook): LocalizedPlaybook {
  const catalog = m as unknown as Record<string, unknown>;
  const read = (suffix: string, fallback: string) => {
    const message = catalog[`growth_playbook_${item.id}_${suffix}`];
    return typeof message === "function" ? (message as () => string)() : fallback;
  };
  const list = (kind: "question" | "output" | "guardrail", values: string[]) => values.map((fallback, index) => read(`${kind}_${index + 1}`, fallback));
  return {
    title: read("title", item.title),
    focus: read("focus", item.focus),
    summary: read("summary", item.summary),
    questions: list("question", item.questions),
    outputs: list("output", item.outputs),
    guardrails: list("guardrail", item.guardrails),
  };
}
...
function HypothesisEvidence({ hypothesis }: { hypothesis: Hypothesis }) {
  return <div className="growth-evidence">
    ...
    {hypothesis.evidence.map((evidence) => <article key={evidence.id}>
      ...
      <p className="growth-muted growth-evidence-meta"><span>{m.growth_evidence_type()}: {evidence.evidenceType}</span> · <span>{m.growth_confidence()}: {evidence.confidence.label}</span></p>
      <p className="growth-muted">{evidence.confidence.rationale}</p>
...
// Rubric evidence is generated by the local evaluator, so it can contain
// counts and measured tokens. Map only those known evaluator sentences; user
// authored evidence and run output deliberately keep their recorded language.
const GROWTH_RUBRIC_EVIDENCE_KEYS: Record<string, string> = {
  "No non-empty <title> element was found.": "growth_rubric_evidence_title_missing",
  "No non-empty meta description was found.": "growth_rubric_evidence_description_missing",
  "The html element does not declare a language.": "growth_rubric_evidence_language_missing",
  "A canonical link with a non-empty href is present.": "growth_rubric_evidence_canonical_present",
  "No canonical link with a non-empty href was found.": "growth_rubric_evidence_canonical_missing",
  "No images are present; there is no image text to audit.": "growth_rubric_evidence_no_images",
  "A non-empty viewport declaration is present.": "growth_rubric_evidence_viewport_present",
  "No non-empty viewport declaration was found.": "growth_rubric_evidence_viewport_missing",
  "The html element declares a non-empty lang attribute.": "growth_rubric_evidence_accessibility_language_declared",
  "The html element does not declare a non-empty lang attribute.": "growth_rubric_evidence_accessibility_language_missing",
  "No images are present; there are no image alternatives to inspect.": "growth_rubric_evidence_accessibility_no_images",
};
function localizedRubricEvidence(value: string) {
  const catalog = m as unknown as Record<string, unknown>;
  const read = (key: string, params: Record<string, string>, fallback: string) => {
    const message = catalog[key];
    return typeof message === "function" ? (message as (inputs: Record<string, string>) => string)(params) : fallback;
  };
  const exactKey = GROWTH_RUBRIC_EVIDENCE_KEYS[value];
  if (exactKey) return read(exactKey, {}, value);
  let match = /^Title contains (\d+) characters; 10–60 is the rubric range\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_title_length", { count: match[1] }, value);
  match = /^Description contains (\d+) characters; 70–160 is the rubric range\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_description_length", { count: match[1] }, value);
  match = /^Found (\d+) non-empty h1 and (\d+) h2 elements; one h1 is required\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_heading_counts", { h1: match[1], h2: match[2] }, value);
  match = /^The document declares lang="([^"]+)"\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_language_declared", { language: match[1] }, value);
  match = /^The archived HTML contains about (\d+) visible text characters\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_content_count", { count: match[1] }, value);
  match = /^Found (\d+) non-empty links that can lead a reader to the next action\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_links_count", { count: match[1] }, value);
  match = /^Found (\d+) images; (\d+) are missing a useful alt attribute\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_images_count", { total: match[1], missing: match[2] }, value);
  match = /^Found (\d+) links or buttons; (\d+) have no visible or explicit accessible name\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_controls_counts", { total: match[1], unnamed: match[2] }, value);
  match = /^Found (\d+) form controls; (\d+) have an explicit label association or accessible name\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_forms_counts", { total: match[1], labelled: match[2] }, value);
  match = /^Found (\d+) external stylesheets and (\d+) parser-blocking scripts; this is a source hint, not a timing measurement\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_loading_counts", { styles: match[1], scripts: match[2] }, value);
  if (value === "No common absolute-outcome claim pattern was found in visible copy.") return read("growth_rubric_evidence_claims_none", {}, value);
  match = /^Found (\d+) possible absolute-outcome claim pattern\(s\); review the surrounding copy and evidence\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_claims_count", { count: match[1] }, value);
  match = /^Found (\d+) main landmark\(s\) and (\d+) labelled navigation landmark\(s\) out of (\d+)\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_landmarks_counts", { main: match[1], labelled: match[2], total: match[3] }, value);
  match = /^Found (\d+) non-empty h1 and (\d+) heading elements; skipped levels: (yes|no)\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_accessibility_heading_counts", { h1: match[1], total: match[2], skipped: match[3] }, value);
  match = /^Found (\d+) images; (\d+) are missing an alt attribute\. Empty alt text is allowed for intentionally decorative images\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_accessibility_images_counts", { total: match[1], missing: match[2] }, value);
  match = /^(desktop|phone): no local render check was archived\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_render_missing", { viewport: match[1] }, value);
  match = /^(desktop|phone): (\d+) × (\d+) PNG target; viewport coverage (confirmed|not confirmed); (\d+) visible text characters; horizontal overflow (detected|not detected)\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_render_check", { viewport: match[1], width: match[2], height: match[3], coverage: match[4], copy: match[5], overflow: match[6] }, value);
  match = /^(desktop|phone): no local timing check was archived\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_timing_missing", { viewport: match[1] }, value);
  match = /^(desktop|phone): DOM ready ([^;]+); load ([^;]+); first contentful paint ([^.]+)\.$/.exec(value);
  if (match) return read("growth_rubric_evidence_timing_check", { viewport: match[1], dom: match[2], load: match[3], fcp: match[4] }, value);
  return value;
}
function localizedRubric(rubric: SeoRubric | PageQualityRubric | RenderRubric | PerformanceRubric | AccessibilityRubric) {
  ...
  return {
    ...
    dimensions: rubric.dimensions.map((dimension) => ({
      ...
      evidence: dimension.evidence.map(localizedRubricEvidence),
    })),
...
function QualityRubricDetails({ rubric, evaluator }: { rubric: SeoRubric | PageQualityRubric | RenderRubric | PerformanceRubric | AccessibilityRubric; evaluator?: string }) {
...
export function GrowthDashboard({ battleId, projectId }: { battleId?: string; projectId?: string }) {
  ...
  const websiteRubric = websiteAudit ? localizedRubric(websiteAudit.rubric) : null;
  ...
  async function downloadReport(format: "html" | "markdown") {
    ...
    await perform(m.growth_busy_report(), async () => {
      const blob = await growth.report(battle.id, format, includeVisuals);
    ...
    <div className="growth-body">
      ...
      <main className="growth-main">
        ...
        {workspaces.isPending ? <div className="growth-empty"><Spinner /><p>{m.growth_opening_local_workspace()}</p></div> : !activeProject ? <>
          ...
          <section className="growth-playbooks" aria-labelledby="growth-playbooks-title"><div className="growth-playbooks-intro"><span className="growth-section-number">{m.growth_playbook_section()}</span><h2 id="growth-playbooks-title">{m.growth_playbook_heading()}</h2><p>{m.growth_playbook_body()}</p></div><div className="growth-playbook-grid">{(playbooks.data ?? []).map((playbook: GrowthPlaybook) => { const latest = playbookRuns.data?.find((run: PlaybookRun) => run.role === playbook.role); const copy = localizedPlaybook(playbook); return <article className="growth-playbook-card" key={playbook.id}><div className="growth-line"><span className="growth-playbook-role">{roleLabel(playbook.role)}</span><span className="growth-playbook-template">{workspace ? m.growth_playbook_executable() : m.growth_playbook_template()}</span></div><h3>{copy.title}</h3><p className="growth-playbook-focus">{copy.focus}</p><p>{copy.summary}</p><details><summary>{m.growth_questions_to_answer()}</summary><ul>{copy.questions.map((question) => <li key={question}>{question}</li>)}</ul></details><details><summary>{m.growth_outputs_and_guardrails()}</summary><strong>{m.growth_outputs()}</strong><ul>{copy.outputs.map((output) => <li key={output}>{output}</li>)}</ul><strong>{m.growth_guardrails()}</strong><ul>{copy.guardrails.map((guardrail) => <li key={guardrail}>{guardrail}</li>)}</ul></details>{workspace && <button className="growth-button growth-button-secondary" disabled={!!busy} onClick={() => void runPlaybook(playbook.id)}>{m.growth_run_locally()} <span aria-hidden="true">→</span></button>}{latest && <details className="growth-playbook-latest"><summary>{m.growth_latest_local_run({ provenance: latest.provenance })}</summary><ul>{latest.responses.map((response) => <li key={response.question}><strong>{copy.questions[latest.responses.indexOf(response)] ?? response.question}</strong><br />{response.answer}</li>)}</ul><p className="growth-muted">{m.growth_playbook_review_next_step({ focus: copy.focus })}</p></details>}<p className="growth-muted">{workspace ? m.growth_saved_local_role() : m.growth_reusable_template()}</p></article>; })}</div></section>
          ...
          <details className="growth-contract"><summary>{m.growth_contract_summary()} <span>{permissionModeLabel(config.permissions.mode)} · {config.validation.commands.length} {m.growth_contract_configured_checks()}</span></summary><div className="growth-contract-grid"><div><h2>{m.growth_contract_allowed_paths()}</h2><p>{config.permissions.allowed_paths.join(", ") || m.growth_contract_none()}</p><h2>{m.growth_contract_denied_paths()}</h2><p>{config.permissions.denied_paths.join(", ") || m.growth_contract_protected_paths()}</p></div><div><h2>{m.growth_contract_validation()}</h2>{config.validation.commands.map((command) => <code className="growth-command" key={command}>{command}</code>)}<p>{config.validation.timeout_seconds} {m.growth_contract_timeout()} · {config.agents.parallelism} {m.growth_contract_parallel_variants()}</p></div><dl><Digest label={m.growth_contract_baseline_commit()} value={battle?.contract.sourceSnapshotCommit ?? workspace?.sourceSnapshotCommit} /><Digest label={m.growth_contract_sha()} value={battle?.contractDigest} /></dl></div></details>
          ...
          {battle && <>
            ...
            <section className="growth-competitors" aria-label={m.growth_competing_variants()}>{variants.map((item, index) => {
              ...
              return <article className={`growth-competitor ${variant?.id === item.id ? "is-focused" : ""}`} key={item.id}>
                ...
                {row?.rubric && <QualityRubricDetails rubric={row.rubric} evaluator={comparison.data?.evaluator} />}
                {row?.quality && <QualityRubricDetails rubric={row.quality} evaluator={comparison.data?.evaluator} />}
                {row?.accessibility && <QualityRubricDetails rubric={row.accessibility} evaluator={comparison.data?.evaluator} />}
                {row?.render && <QualityRubricDetails rubric={row.render} evaluator={comparison.data?.evaluator} />}
                {row?.performance && <QualityRubricDetails rubric={row.performance} evaluator={comparison.data?.evaluator} />}
            ...
            {!!status.data?.runs.length && <section className="growth-share"><div><span className="growth-eyebrow">{m.growth_battle_take_evidence()}</span><h2>{m.growth_battle_report_title()}</h2><p>{m.growth_battle_report_body()}</p></div><div><label className="growth-checkbox"><input type="checkbox" checked={includeVisuals} onChange={(event) => setIncludeVisuals(event.target.checked)} />{m.growth_battle_include_visuals()}</label><p className="growth-muted growth-report-note">{m.growth_battle_include_visuals_note()}</p><div className="growth-report-buttons"><button className="growth-button growth-button-secondary" disabled={!!busy} onClick={() => void downloadReport("html")}>{m.growth_battle_download_html()}</button><button className="growth-button growth-button-secondary" disabled={!!busy} onClick={() => void downloadReport("markdown")}>{m.growth_battle_download_markdown()}</button></div></div></section>}
          </>}
        </>}
