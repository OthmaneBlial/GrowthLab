...
function HypothesisTree({ hypotheses, goal, audience }: { hypotheses: Hypothesis[]; goal: string; audience: string }) {
    ...
    <ol className="growth-hypothesis-branches" aria-label="Growth hypothesis branches">
      {[...groups.entries()].map(([role, items]) => <li className="growth-hypothesis-branch" key={role}>
        <details open><summary><span>{roleLabel(role)}</span><small>{items.length} hypothesis{items.length === 1 ? "" : "es"}</small></summary>
          <div className="growth-hypothesis-children">{items.map((item) => <article key={item.id} className="growth-hypothesis-node"><div className="growth-line"><span className="growth-playbook-role">{roleLabel(item.role)}</span><Mark value={item.provenance} /></div><h3>{item.title}</h3><p>{item.hypothesis}</p><p className="growth-muted">Mechanism: {item.mechanism}</p><small>{item.channel} · metric {item.primaryMetric}</small></article>)}</div>
        </details>
      </li>)}
    </ol>
  </div>;
